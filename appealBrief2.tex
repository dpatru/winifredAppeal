...
Under the Tribunal's interpretation if the assessor overassessed the property in the year before the repairs, as is likely the case here, then the home owner can not benefit from the repairs taxwise because the assessment would be based not on the repairs, but on the previous year's inflated assessment. Here Appellant did not even own the property when it was overassessed before it was repaired; he could not have appealed its assessment himself. Thus here the Tribunal would condition the statute on the previous owner's decision to not appeal the previous year's assessment. At the time Appellant purchased the property the time for appeal had passed, and therefore, under the Tribunal's reasoning, \mathieuGast\ was a dead letter.
%The Tribunal implicitly assumes that the previous year's assessment is correct, and that The Tribunal does not examine the correctness of the assessment, it just refuses to apply nonconsideration treatment if the increase from the the previous year's assessment could be caused by inflation.
...
% Whether the assessor subjectively thought about the repairs is irrelevant.
...
%\mathieuGast\ is not concerned with the minds of assessors, nor does it require that past assessments be assumed correct without proof. Instead, the statute is focused on making sure that the contribution of normal repairs are quantified, noted in the assessment rolls, and explicitly removed from the assessed value. Nonconsideration treatment requires the assessor and, in case of appeal, the Tribunal, to determine based on evidence, the 
...
Regarding normal repairs under \mathieuGast\ the \STC\ requires that ``the true cash value of the item shall be calculated by performing `before' and `after' appraisals and then deducting the `before' true cash value from the `after' true cash value.'' \pincite{STC Bulletin}{2}. Also in ``Instructions to Assessor for Processing Form 865'' the \STC\ writes:
...
property is not is presumptive true cash value. 9 Despite Petitioner's assertions that the
...
\footnote{In support of its rejection of the sale price, the tribunal also cites \cite[s]{MCL 211.27(6)}, prohibiting assessors from presuming that the selling price is the true cash value. But this but, as noted in the quote above, \cite[s]{Jones & Laughlin}\ specifically consider and reject this as justifying a cursory rejection..}%
\reconsiderationDenied[2].
...
On 4/1/2019, I served a copy of this Brief on Appellee's counsel by electronic service.
